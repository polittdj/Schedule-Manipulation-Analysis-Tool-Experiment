<# =====================================================================================
 Polaris² (Schedule Forensics) — one-file installer ({{TIER_LABEL}})
 =====================================================================================
 ...
 reinstalled):
   ...
   3. Installs the Polaris² tool into its OWN venv from the wheel embedded
...
Write-Host ""
Write-Host "Polaris² (Schedule Forensics) installer — v{{WHEEL_VERSION}} — $TierLabel" -ForegroundColor White
...
Step "Creating the Polaris² shortcut, uninstaller, and first-run README"
...
@"
# Uninstall Polaris² (Schedule Forensics): removes the app folder + shortcuts.
...
`$menuOld = Join-Path ([Environment]::GetFolderPath('StartMenu')) 'Programs\Schedule Forensics'
Remove-Item -Force -ErrorAction SilentlyContinue (Join-Path `$desk 'Polaris².lnk')
...
Remove-Item -Recurse -Force -ErrorAction SilentlyContinue `$menuOld
Remove-Item -Recurse -Force '$InstallRoot'
Write-Host 'Polaris² removed.'
"@ | Set-Content -Path $uninst -Encoding UTF8

@"
Polaris² (Schedule Forensics) — first run
=========================================
Start:  double-click "Polaris²" on the Desktop (the ONE icon).
...
foreach ($old in @("Start Schedule Forensics.lnk", "Stop Schedule Forensics.lnk", "Schedule Forensics.lnk")) {
...
foreach ($dir in @($desk, $menu)) {
    $lnk = $shell.CreateShortcut((Join-Path $dir "Polaris².lnk"))
...
Stop-Transcript | Out-Null
Write-Host ""
Write-Host "DONE — double-click 'Polaris²' on the Desktop to begin." -ForegroundColor Green
