...
| **F-14** | F-set | LOW | none | **CLOSED** (ADR-0153 — swept against the delivered handbooks: the 10-d float band is SOURCED to PPC SP-2016-3424 Fig. 3.4-3 p.138; the secondary/tertiary tier practice to PPC §3.4.3.2D/SOPI 6.0 p.37/SRB SP-20230001306; the 35% lag ratio and merge-link count have NO published value and stay documented design choices) | `driving_slack.py`, `health_extra.py`, `float_erosion.py` provenance notes | CONFIRMED | closed in-env |
