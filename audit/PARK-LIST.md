...
| A-8 threshold authority | **CLOSED (ADR-0153)** — the F-14 sweep ran against the delivered set: 10-d float band SOURCED (PPC SP-2016-3424 Fig. 3.4-3 p.138 ≤ 10 d Total Slack screen); secondary/tertiary tier practice SOURCED (PPC §3.4.3.2D p.151, SOPI 6.0 p.37, SRB SP-20230001306); 35% lag ratio + merge-link count: no delivered handbook publishes a value — documented design choices at point of use. NOTE: the NASA Schedule Management Handbook (SP-2010-3403) itself is NOT in the intake; if delivered later, re-run the sweep for the two unsourced numbers. |
...
| A-8 | **Structural/health threshold authority** (F-14) — CLOSED per ADR-0153 (see status table above); only SP-2010-3403 could still improve the 35%/merge-count numbers | delivered (partial: SP-2010-3403 absent) | Done: cited at point of use; 2 values documented as design choices. |
