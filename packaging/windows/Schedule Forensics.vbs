...
'
' Requires the tool installed so that "pythonw -m schedule_forensics" resolves on
...
'
' Target is the package entry point "-m schedule_forensics" (not "-m schedule_forensics.launcher")
' so that a startup failure under pythonw - a rebuilt venv, a missing dependency - is caught by
' the guarded bootstrap and shown in a message box, instead of dying silently on a dead port.
Option Explicit
Dim sh
Set sh = CreateObject("WScript.Shell")
' window style 0 = hidden (no console), False = don't wait for it to finish
sh.Run "pythonw -m schedule_forensics", 0, False
