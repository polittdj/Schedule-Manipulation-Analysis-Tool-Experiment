...
**ADR-0515 (R-04 — 2026-09-20): Acumen Fuse's own rounding is half-to-even wherever its code rounds; the sweep of the tool's `round()` sites toward `round_half_up` is refuted by measurement.** The three Forensic Analysis Reports with a committed golden on both sides (the Large Test File pair, the two Hard_File pairs) print every whole-day field — Original Duration, Remaining Duration, Total Float — half-to-even on every comparable row (LTF 752 / 773 / 1,297; half-day ties of both parities, 20 / 11 / 262 on the first snapshot; half-away-from-zero misses exactly the even-part halves), the day change as the difference of the ROUNDED fields (the rounded exact difference reproduces only 1,216 of 1,297), the 2-dp ratio as ToEven on the scaled value (1/8 → 0.12, 9/8 → 1.12, 17/40 → 0.42, 1/40 → 0.02; half-up refuted on all 11 discriminating ties) and the Start / Finish change as the calendar-day difference truncated toward zero (2,741 / 2,741). The metric values in the Summary / History reports are stored RAW under Excel's General format, so no Fuse-displayed metric value at a tie exists to measure MF-08's half-up against. Pinned by `tests/parity/test_fuse_forensic_rounding_oracle.py` (red by name under three mutants of the rule); the three Hard_File rows the rule does not reproduce are a DIVISOR (the activity's own calendar day — R-75), and the one LTF duration row is the model's minute grid (UID 5267, `PT107H59M36S`), both named in the oracle. No engine figure moved; the 374 `round()` sites are classified by exposure in `tests/guards/round_site_ledger.tsv`.
