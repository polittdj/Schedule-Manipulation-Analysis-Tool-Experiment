...
- **Shipped:** the rule (`engine/cpm.py`); `tests/engine/test_backward_pass_past_finished_work.py` (13 pins — 11 red on the pristine engine by name, 2 controls named as such); the stored-dates oracle (`tests/parity/test_hard_file_stored_dates_oracle.py`: the late-finish census counts INCOMPLETE work only, the floors raised, the row's witness pinned); six pins re-derived with dated reasons (`test_actual_start_floor_own_calendar`, `test_resume_floor_total_float`, `test_float_analysis`'s Project2 count, `test_recorded_completed_window`'s DCMA-12 premise, now on a rig; and, after CI's floor job named them, the wide program's negative-float counts in `test_pass_fail_battery` — 16 → 0 and the MFO sink 38 → 22 of 38, a finished merge point presenting no late need — and `test_ch01_critical_basis`'s premise, whose witness is now synthetic because the engine's pure-logic flag agrees with MS Project's on every incomplete activity of all five Hard_File goldens); the report's R-70 row closed and R-71 / R-72 registered; `docs/PARITY-REPORT.md` re-measured.
...
- **Six pins moved, each re-derived with its reason** (four found by the shadow run, two by CI's floor job on the first head — the wide program's counts through its completed merge point, and CH01's witness): the own-calendar floor's predecessor reads
