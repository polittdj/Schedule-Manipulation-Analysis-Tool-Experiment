...
| R-60 | T2 | leveling SPLITS live only in the `.mpp`'s timephased data — Hard_File_updated3 UID 403 is two work pieces seven days apart (MPXJ `getWorkSplits`: 10-19 15:00 → 10-21 12:00, then 10-28 13:00 → 10-30 15:00 in the intake save), Hard_File's UIDs 14 / 401 the same class — and the MSPDI the converter writes carries no `TimephasedData` and no split element, so the engine runs the pieces contiguously (403: 10-23 vs the stored 11-05; its chain of four) (ADR-0487) | OPEN | M | have the vendored converter emit MSPDI `TimephasedData` (`MSPDIWriter.setWriteTimephasedData`), read each assignment's zero-work gaps in the importer, and honour them as elapsed gaps inside the task the way the leveling delay is honoured; red-first on 403's stored 2026-11-05 09:12 with the corpus re-measured (the fixtures must be regenerated from the SAME saves) | 403 and its chain within a day, Hard_File's 14 / 401 spans, everything else unmoved |
| R-61 | T2 | a FIXED_DURATION booking on an off-pattern crew spans the DURATION in crew minutes (ADR-0474's type rule) where MS Project keeps the task's window: updated3 UID 210's Content Developer booking is recorded over four project days and computed as 1.33 days on the 24-hour calendar — masked today by the completed-window pin (ADR-0476) and by the material leg (ADR-0487) | OPEN | S | census every FIXED_DURATION task with an off-pattern crew across the goldens, recorded booking window vs the computed leg; if the window is the rule, a fixed-duration leg spans the task's window on the crew calendar | the stored booking windows of those tasks |
