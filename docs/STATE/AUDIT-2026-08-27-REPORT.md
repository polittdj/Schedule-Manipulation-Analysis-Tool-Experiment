...
| R-57 | T2 | assignment-level `LevelingDelay` (Hard_File UID 398's two bookings, UID 188) is not read — only the task's delay is honoured (ADR-0474). **CLOSED 2026-09-16 (ADR-0502) — v1.0.268.** The mechanism is real and the row's oracle is reached, but TWO things in the row were wrong and both were measured before anything changed. **The population is wider than the row:** six of the fifteen goldens carry the field — **24 delayed bookings on 17 tasks**, every `Large_Test_File` snapshot included — not the two Hard_File activities named. **"Delay that leg alone" is right for 6 of the 24 and a REGRESSION for the other 18:** implemented as written it cost `Large_Test_File` **93 of its 1,666** finishes-within-a-day and took UIDs 5266 / 5267 / 5270 from EXACT to days late. The delay rides **ADR-0474 / ADR-0501's type axis**: a leg with its OWN span (`FIXED_UNITS`, ratio < 1) is **PUSHED** by its delay, a leg that **SPANS THE TASK** (ratio 1.0) **ABSORBS** it — the delay lies inside the span it shares with the task, and on **16 of those 18** the file's own `Assignment/Finish` IS its `Task/Finish`. Split 6 push / 18 absorb, exactly on the file: pushed = Hard_File, absorbed = Large_Test_File. The delay runs in WORKING minutes of the leg's own calendar, is read to the NEAREST minute (17 / 24 against MS Project's own `Assignment/Start`, truncation 14, never worse on any booking; all 7 misses sub-minute, 12–78 s, R-65's class), joins the leg's dedup IDENTITY (or a leveled crew collapses onto its unleveled twin), is never read on a MATERIAL / COST booking (ADR-0487: its leg IS its window), and does NOT move the task's start (`Task.Start == min(Assignment.Start)` on 17 / 17). Disclosed on `assignment_leveling_driven` only when the delayed leg PLACES the finish, measured from the task's real early start — the first cut used the project-start sort order and falsely claimed updated2's UID 398, whose delay is inert. **Measured:** every golden BYTE-IDENTICAL except `Hard_File_updated` (exact finishes 58 → 61, exact stored slack 93 → 99); UID 398 on updated → **EXACT 2026-08-27 11:59**; UID 188 15 h 45 m early → **66 seconds**. On the base `Hard_File` snapshot 398 is NOT exact and the blocker is upstream, not leveling: **UID 381 finishes a day early there**, 396 inherits it, and 398 lands early by exactly the 240 working minutes 396 is (computed in the pin) — registered as **R-66**. Battery 9 / 9 red by name; **M6 (the BACKWARD pass) SURVIVED the first pass** and is worth three exact stored slacks (UIDs 321 / 381 / 396), killed by a pin naming them. A post-delay snap was DELETED rather than shipped: swept over 31,479 combinations, 6 landing exactly on a segment end, it never moved a leg finish | CLOSED | — | closed 2026-09-16 (ADR-0502) | — |
...
| R-61 | T2 | a FIXED_DURATION booking on an off-pattern crew spans the DURATION in crew minutes (ADR-0474's type rule) where MS Project keeps the task's window: updated3 UID 210's Content Developer booking is recorded over four project days and computed as 1.33 days on the 24-hour calendar — masked today by the completed-window pin (ADR-0476) and by the material leg (ADR-0487). **CLOSED 2026-09-16 (ADR-0500) — REFUTED, `engine/` untouched:** the census this row asked for was run over all 15 goldens and the POPULATION IS ONE TASK — UID 210 in five snapshots of one file, no unstarted witness and none without a project-calendar co-booking, so no golden can discriminate the rule. The row's arithmetic is right (1,920 crew minutes vs a 7,740-minute recorded window, the booking's window being the TASK's own) but its premise is mis-stated: the finish-placing PRIMARY leg is the `Standard`-calendar WORK leg of the Logistics Apprentice booking, landing on the stored finish exactly, and the crew leg — four days earlier — has never placed the task. Cutting ADR-0476's pin reads 210 **+1,260 min LATE**, not early; cutting ADR-0487's material leg moves 210 not at all (it moves the file's finish 12-12 → 12-06, so the cut has teeth). The proposed rule, implemented on a shadow engine, FIRES (ratio 1.0000 → 4.0312, span 1,920 → 7,740) and changes NOTHING: every figure on all 15 goldens byte-identical, its only effect a lost `booking_span_driven` disclosure. Generalized — MS Project has no separate scheduler for FIXED_DURATION — it moves **315 activities AWAY from their files' own stored finishes and ZERO toward**, because a split booking's window spans the leveling gaps ADR-0491 honours separately (UID 5231's window is 26,880 crew minutes SHORTER than the correct occupancy). The one alternative the row does not name — a fixed-duration leg on the TASK's axis — is 0 toward / 5 away. 10 pins shipped (census, witness, corrected premise, five stored-finish oracles, the ADR-0491 refutation witness, one labelled tripwire); battery 6 / 6 red by name, and it caught this unit's own first cut, a census that re-implemented the plan builder and was green under the mutant that breaks it  **SETTLED 2026-09-16 (ADR-0501) on the repository's own 29 `.mpp` files — roughly TWICE the golden population, including files no golden was made from.** ADR-0500's residual was mis-scoped: it named "a production IMS" when what it had measured was the GOLDENS. The wider census upholds ADR-0474 and DISCHARGES the residual: of **483** tasks placed by an off-pattern ratio-1.0 crew leg, the recorded window and the engine's occupancy (duration + ADR-0491 gaps) are **byte-identical on 369**; nearly all the rest differ by **1–28 MINUTES** (gap granularity, registered as R-65, not a different rule); on the 18 UIDs a leg-alone probe matches neither, the **shipped solve is within a day on 17**; and the corpus's ONE decisive case — `Large_Test_File2` UID 5263, *started* so its stored Finish really is a schedule, window 9,600 min past the duration, leg alone four weeks early — is **EXACT in the shipped solve**, disclosed on `date_driven`: a stored date places it, not the leg. **Two corrections to ADR-0500, both measured:** its refutation witness UID 5231 is recorded-COMPLETE, so its stored Finish is its ActualFinish — a record, not a schedule — and adjudicates nothing; and the engine's leg does NOT "get it right" there but lands **eighty days late**, the solve being exact only through ADR-0476's pin. ADR-0500's "315 away / 0 toward" stands (a different, valid measurement); the sentence explaining UID 5231 does not | CLOSED | — | closed 2026-09-16 (ADR-0500), settled 2026-09-16 (ADR-0501) | — |
| R-65 | T2 | the engine's honoured leveling gaps (ADR-0491) land **1–28 minutes** short of the window MS Project records for the same split booking — systematic and one-directional across ~100 bookings in the 29-file `.mpp` corpus (UID 5342 occupancy 2,757 vs window 2,758; UID 5316 9,329 vs 9,333; UID 5273 4,687 vs 4,688). Below the resolution of every parity pin in the repo and it moves no verdict, but it is real and it was folklore until ADR-0501 measured it | OPEN | S | compare `_split_gaps`' minute sum against the file's own recorded window on the ~100 split bookings where they differ; decide whether the residual is rounding in `_recorded_span`, in the share→`after` conversion, or in MPXJ's timephased block boundaries | the recorded window of those bookings |
| R-66 | T2 | Hard_File's **base snapshot only**: UID 381 finishes a full working day early (2026-08-20 11:59 against the stored 08-21 11:59); 396 inherits it and 398 lands early by exactly the 240 working minutes 396 is, which is the entire reason R-57's oracle is reached on `Hard_File_updated` and not on `Hard_File` (ADR-0502). ADR-0474's task-delay arithmetic is EXACT on BOTH snapshots when fed the stored predecessor finish, so the defect is upstream of leveling; the same chain is exact on updated, so it is snapshot-specific, not structural | OPEN | S | trace 381's own predecessors on the base snapshot against its stored finish, red-first on 381's 2026-08-21 11:59; then re-measure 396 / 398 (the R-57 pins already compute the 240 and will move with it) | UID 381's stored finish, then UID 398's on the base snapshot |
...
| key | value | method |
| --- | --- | --- |
| `round_calls_outside_engine_metrics` | 329 | AST `Call` nodes whose callee is the bare name `round`, every `.py` under `src/schedule_forensics/` except `engine/metrics/` (44 files carry one); 325 → 327 on 2026-09-07 (ADR-0474): the plan builder's two span roundings in `engine/cpm.py` (`round(ratio * dur)`, the remaining-work scaling); 327 → 328 on 2026-09-14 (ADR-0491): the split gap's place in a scaled leg, `round(share * span)` in `_execution_plans`; 328 → 329 on 2026-09-16 (ADR-0502): the MSPDI reader's `round(delay_tenths / 10)` — a BOOKING's leveling delay is stored in tenths of a minute and rounding reproduces MS Project's own `Assignment/Start` on 17 of the goldens' 24 delayed bookings against truncation's 14 |
