...
**What is still open, in one sentence per tier (the full pricing in §3):** T1 — one engine
default that flatters CPI on a mixed cost-loaded file, one parity-locked rounding rule awaiting an
Acumen oracle, one census of 325 displayed roundings to classify, and one baseline-basis question
for P6 that is the operator's; T2 — thirteen page scripts that explain a render crash as a data
...
| **UI-03** | WP8 · ADR-0472 (NEW, this report) | OBSERVED and ATTRIBUTED — every page scrolls horizontally at 1440 (headless): the hidden tooltip box | R-20 (OPEN, S) |
...
| --- | --- | --- | --- | --- | --- | --- |
| R-01 | T1 | `evm.py`'s ACWP `actual_cost or 0.0` on a mixed cost-loaded population (WP6's own row) — a blank actual counted as 0 spent makes CPI and TCPI read BETTER than the truth | OPEN | S-M | a two-task cost-loaded fixture with one `actual_cost=None`: a red test that CPI/TCPI read NA-or-disclosed (an `actuals_missing_count`, the shape JCL's `actuals_assumed_count` took in ADR-0463 MC-03) instead of treating the blank as zero | the Fuse EVM export on a mixed-actuals file settles the parity rule: blank-as-zero, or excluded |
| R-02 | T1 | IMP-05 — on a P6 XER the per-task baseline dates ARE the PLANNED (target) dates; disclosed by an import note, values kept | ASK | S | operator question (b): keep the disclosed planned-date basis or read N/A until a baseline project is exported; the None branch is one function under the existing note | a Fuse export on an XER settles the parity leg |
...
| R-12 | T3 | TX-03 — the model dropdown's catalog probe transmits a body-less, key-bearing `GET /v1/models` BEFORE the acknowledgment and is recorded (ADR-0402's design, disclosed) | ASK | S | operator question (a); if gated: the model select disabled until the acknowledgment and the route refusing before consent, red-first with WP7's existing opener spy | the operator's answer |
...
| R-20 | T4 | UI-03 (NEW) — every page's document scrolls horizontally ~280–300 px at a 1440-px viewport (`document.scrollingElement.scrollWidth` 1719 on `/`, 1734 with the family's strip): the hidden tooltip box `[data-sf-hint]::after` (340 px, `position:absolute; left:0; visibility:hidden`) on right-aligned hosts — the Reset-view button in every `.viz-controls` row — counts in scrollable overflow; `body` overflow-x is `visible`; 75 hint hosts on /wbs; headless hides the scrollbar the operator would see | OPEN | S | render the box only on `:hover` / `:focus-visible` (`display:none` otherwise) and pin `document.scrollingElement.scrollWidth <= innerWidth` across the M1 census's 34 page states — red-first on this tree in all four themes | the pin green in four themes and a headed check that the scrollbar is gone |
...
| R-23 | T4 | T-01 — "the timeline doesn't change when I tell it to" — MEASURED WORKING on v1.0.233+, NOT REPRODUCED, two pins added | ASK | — | operator question (e): which page, which zoom, a screenshot of the dialog beside the header | the screenshot |
| R-24 | T4 | I-01 — the integrity page's findings, ROOT-CAUSED to the session population (the page now names its population and says "nothing to compare") | ASK | — | operator question (e): which finding disappeared, on which two files, dropped as separate folders or carrying different Titles | the two files |
...
| R-29 | T4 | /forecast's chips with two files · /trend's chips · the parent-folder question on `/` | ASK | — | operator question (e): the migrated pages' controls on their own files | their report |
| R-30 | T4 | CF-01's follow-up — #635's /integrity working-day move for UID 152 | ASK | — | operator question (d): on v1.0.236+ does the working-day count read right against the file's calendar | their reading |
| R-31 | T4 | /onepager-compare's three rulings (swimlane move · threshold · one vs three slides) and its `.pptx` in PowerPoint (ADR-0465) | ASK | S | operator question (c); each ruling is a one-place change (`item_key` pairing, a threshold constant, a three-slide render) | their rulings; the `.pptx` opened |
...
| R-42 | T6 | the design migration queue — /compare (10) is a feature (M-L); /standards and /scorecards next; 22 artboards remain (§6) | ORG | — | one page per session (the standing ask); /compare needs the cross-pair evidence ledger ("All five pairs × every edit") and a slip decomposition update by update that no page computes | the operator's order |
...
| key | value | method |
| --- | --- | --- |
| `round_calls_outside_engine_metrics` | 325 | AST `Call` nodes whose callee is the bare name `round`, every `.py` under `src/schedule_forensics/` except `engine/metrics/` (44 files carry one) |
| `round_calls_inside_engine_metrics` | 45 | the same walk inside `engine/metrics/` (MF-08's `round_half_up` sites are not counted — they are not `round`) |
...
**Done (8):** /volatility (04) · /cei (06) · /trend (05) · /forecast (09) · /onepager-compare
(Library One-Pager Timeline, built new on the design) · /performance (07) · /card (Library Schedule
ID Card) · /wbs (Library WBS Rollup, ADR-0471, this session).

**Priced as a feature, not a migration:** /compare (10 What changed) — M-L (R-42).

**Remaining (22 artboards):** 00 Import · 01 Where we stand · 02 Can we trust the plan · 03 What
drives the date · 08 Who is overloaded · 11 What could go wrong · 12 The briefing · Control Margin
Dashboard · Control Standards and Execution Indices (/standards — next by cost) · Control Assessment
Scorecards (/scorecards) · Forensics Schedule Integrity · Library Beyond the Schedule · Library EVM ·
