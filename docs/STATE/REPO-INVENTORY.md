...
> material (incl. the `.xer` samples), and the build/CI/installer toolchain. Read this first to recall
> "what is here and how to use it." It is assembled from a parallel read-only inventory of the whole
> tree. `docs/STATE/HANDOFF.md` remains the live "where we are / what's next"; this is the standing map.
...
**`margin.py`** — schedule margin (buffer named "margin"). `is_margin_task(task)`, `compute_margin(schedule, cpm) -> MarginAnalysis` (total vs **effective** margin = zero every margin task's duration, re-run CPM, measure finish pull-in — via `duration_overrides`), `compute_margin_trend(versions) -> tuple[MarginPoint,...]`. Parity-isolated.
...
Only *source-of-truth* fields; **CPM dates/float/DCMA/EVM are derived by the engine, never stored** (`_base.py` docstring). `SCHEMA_VERSION = "2.6.0"` in `__init__.py` (bumped on any field change; freeze test enforced).
...
| Web/UI | `tests/web/` | 102 | the single-file FastAPI app (`web/app.py`) — every route/page/panel, chart contract, drill tables, i18n, air-gap/CSP, upload resilience & caching, single-compute chokepoint, accessibility, themes, SRA views, export endpoints; JS harnesses under `tests/web/js/*.mjs` run through `node` |
...
| Exhibits | `tests/exhibits/` | 2 | the exhibits render/CLI layer + an SSI-token-ambiguity lint gate |
...
- **`HANDOFF.md`** (~372 KB) — read-first single source of truth for "where we are / what's next," newest handoff on top followed by prior handoffs as a running stack. **Current status:** version **1.0.39**, **highest ADR 0227** (2026-07-15). The latest entry is an operator bug-fix cluster (ADR-0227): folder-upload resilience (fetch + per-file `arrayBuffer()` pre-read; OneDrive-placeholder / file-open-in-Project files skipped-and-named rather than aborting the whole upload), `/path` version agreement (schedule `<select>` defaults to latest version so header and grid describe the same file), driving-slack scope fix (`st.scope(sch)` on `/api/driving*`), inactive-task baseline exclusion, and a new SSI-leveled parity gate pinning the critical path to UID 152 (60/60) + full driving set (783/783) against the operator's real `Large Test File Leveled.mpp`. Investigation confirmed the driving-slack/critical-path **engine was correct** (reproduced SSI 783/783); bugs were in the web/UI layer + one importer rollup. Gate reported green. **NEXT:** larger new features as separate PRs — MS Project saved filters/groups/highlighting + A–Z ordering, Gantt click-to-highlight, then F3a/3b margin work.
...
Recent version arc visible in the HANDOFF stack: 1.0.34→…→1.0.39; the **SMAT v4** feature line (grouped ingestion + Portfolio F1/ADR-0225 → scale/SQLite-cache/RAM/batch-JVM F2/ADR-0226 → operator bugfixes ADR-0227), with margin F3a/b, roles F4, F3c still ahead.
...
- **`RTM.md`** — Requirements Traceability Matrix. Every §6.A–§6.G requirement + units §3 + QC/PM §7 → module → test → parity evidence → milestone → status. Almost every row reads **✔** (Implemented+Tested+Validated). Notable evidence rows: B2 parity "SSI 107/107; Acumen §A; §B 13/14; §C counts+BFC; §E … Net Finish Impact −99; residuals formally accepted+locked (ADR-0014)"; overall coverage cited at 99%.
...
### `docs/adr/` — 228 files (ADR-0000 through ADR-0227), no separate index
...
**Cross-cutting drift guard:** `tests/test_state_docs.py` fails unless the highest ADR number on disk (currently **0227**) appears in both `HANDOFF.md` and `SESSION-LOG.md`, and a doc-sync test enforces `METRIC-DICTIONARY.md` regeneration from `web/help.py`.
...
**Key numbers:** MPXJ 16.2.0 (25 jars, ~17 MB); 9 installers × ~1.27 MB; app port 8321; batch JVM heap cap 1g (`SF_MPXJ_XMX`); conversion timeout 300 s; JVM boot timeout 60 s; CI on Python 3.11 + 3.13; coverage gates overall ≥70% / engine ≥85%.
