...
- **When you pin an oracle, grep the register for the rows it answers.** R-48's own "first executable step" — a Fuse ribbon on a file with a completed activity over 44 baseline days — was executed by ADR-0518's 87 / 927 pin eleven days before this session, on a file that carries 77 such activities (164 / 1,569 if the tile admitted them). Nobody re-read R-48 because the pin landed under R-76. A row must name the oracle it waits on, and the session that lands that oracle owes the register a grep.
- **A count that matches is not yet a metric; the population is half the definition.** R-51's count reproduced Fuse's 68 / 65 / 47 / 41 under the planned-or-in-progress rule with or without milestones in the population; only the RATIO (0.80 against 0.62 on Hard_File) separated the two populations. Pin the ratio beside the count whenever the reference prints one.
- **Name the instrument that sees the clause a surviving mutant hides.** Dropping the milestone clause left the whole parity oracle green — no committed save carries an estimated milestone — and only the hand-built unit test went red. The right response was to record which instrument guards the clause, not to fabricate a fixture so the oracle could "see" it.
- **A citation cap turns set-equality into subset-and-count.** The health check cites at most 50 offenders; the 65-mark sheet cannot be compared as a set. Assert what the cap allows (subset, equal count, set-equal where the count fits) and say why in the docstring, rather than raising the cap for a test.
- **Fuse's writer omits `r` on consecutive cells** (ADR-0516 M10, re-paid): the first X-mark extractor crashed on a `<c>` with no reference. A document-order reader that slides columns is the only correct shape; the committed oracles already carry it — copy it, do not rewrite it.
- **Check the binary, not the exit code (again).** `ruff` on PATH here is 0.15.8; CI resolves 0.16.9, which also formats fenced python inside Markdown. The full-gate skill names the trap; the ADRs were written without python fences and checked with the absolute path.
...
- **A substring locator is a test that passes by accident.** `filter(has_text="6")` matched five rows and clicked WBS 1; the overlay read 7 against 6 and the count caught it. Match a row by its header exactly.
- **When the plan needs a rule the corpus stores the answer to, read the stored field, not the derivation.** Whether MS Project's Total Slack on started work is min(start, finish) or the finish slack alone was settled by two elements nobody had censused: StartSlack 0 and TotalSlack == FinishSlack on 1,159 of 1,159. The derivation (restart ≥ AS) agreed, but the file is the evidence.
- **One counter-witness refutes a rule; it does not become a residual.** The clamp reproduced 22 of 23 negative-finish-slack started activities; UID 187 stores LF 3.5 days below EF with the same negative slack. "22 of 23" is not a rule with an exception, it is an unknown rule — leave it unbuilt and name the witness.
- **A settle criterion without its instrument, sequence and box is not a criterion.** "p95 ≤ 50 ms" was already met on the wheel sequences without any change and unreachable on the re-aim sequence with every sticky cell stripped. Commit the probe first; then price.
...
- **What happened.** R-60 (leveling splits) was built as specified — the converter writes the
  timephased data, the importer reads a WORK booking's zero-work blocks as its pieces, the engine
  honours the gaps — and the first corpus census read updated3's UID 403 exact, every Hard_File
  snapshot's finish exact, and **24 activities on Large_Test_File2 three weeks LATE**. The chain
  head was UID 5308: a fixed-duration task of 228 h with eight bookings, one of which records a
  three-week zero window the other seven work straight through; MS Project's Duration already
  spanned it. UID 5265 — one booking, daily gaps excluded from its duration — landed exact under
  the same rule. The discriminator is not the booking's data but the TASK's: a gap nobody works
  is the task's split (Duration excludes it), a gap another booking works through is that
  booking's own contour (Duration includes it). Two unit tests now pin both sides; 18 / 18 mutants
  red by name.
- **The lesson.** A file's timephased fact has an OWNER. "Read what the file records" answered
  what the gap was; it did not answer whose duration it lived inside. Census the whole corpus
  with a per-task before/after and a stored-finish direction (toward / away) BEFORE writing the
  ADR — a golden-level count ("finish within a day 1589 → 1605") hid 24 regressions behind 80
  improvements on the same file. The away-mover whose predecessors are not away-movers is the
  rule's counter-example, and it is found by derivation, never by reading.
- **Provenance is a diff, not a report.** ADR-0487 settled the fixture's save by the Fuse
  report's figure and wrote "the same save is not available". `git log --all` found it (blob
  `1908d073`); converting every historical blob of every intake `.mpp` and diffing the task and
  assignment sections matched 14 of 16 goldens to one save each — and showed the converter's
  `CurrentDate`, `MaxUnits`, `AvailableFrom` / `To` and rates are wall-clock artefacts (R-63). A
  golden regenerated by SPLICE (the old file plus the new `<Assignments>` section, with the proof
  that nothing else moved inside the tool) inherits none of them.
- **The rig can be wrong before the engine is.** The first green refuted the TEST: two pieces
  declared as 240 minutes over a 480-minute window; the engine trusts the work minutes. Recompute
  every expectation from the rig's own numbers before touching the code.
- **`--follow` invents history.** `git log --follow` on the intake path listed two commits that
  never carried the file (rename detection); `git log -- path` without it did not. Address a
  save by its BLOB (`git cat-file blob`), never by a path at a commit.
- **The compiler is the API reference.** `getSplits` / `getTimephasedWork()` / `getAmountPerDay`
  do not exist in MPXJ 16.2.0; `javac` refused them in one second, `javap` gave the real names
  (`getWorkSplits`, `getRawTimephasedRemainingRegularWork`, `getAmountPerHour`). Never write a
  probe from memory of a library.

### 2026-09-14 — a "teeth" pin ages with the engine, and a register's red-first can be impossible on every fixture you own (ADR-0489 / ADR-0490)

- **The register said "red-first on Fuse's Zero Days Float 66 / 2"; the tree said no.** On every one of the 28 MSPDI fixtures the 109 Critical activities with an absent `TotalSlack` already recompute to exactly 0 minutes, so the stored-preferring rule and its fallback agreed to the minute and the Fuse figure was exact before a line changed. A pin written against that figure would have been green on both trees and proven nothing. What was done instead: a census of the whole corpus for the case that DISCRIMINATES (Hard_File UIDs 241 / 249 — pure logic 480 / 360 where MS Project stored 0 — the only two in 28 files) plus a synthetic file where the bases disagree by three days. **Before writing a red-first pin, ask whether any fixture you own can go red; if none can, build the one that can and say so.**
- **ADR-0430's "teeth" had fallen out two months ago and nobody noticed.** Its pin asserted that Hard_File carries 34 Critical activities with no stored slack "so that a reintroduced recompute fallback re-counts 34 negatives" — true on the 2026-07 engine. ADR-0474 (crew calendars) made those 34 recompute to 0 (32) and +480 / +360 (2): never negative, so a reintroduced fallback would count nothing and the pin could not fail for its stated reason. It stayed green because green is silent. **When an engine rule changes, re-derive every pin that cites the OLD behaviour as its reason — a pin's justification is a claim about the engine, and it goes stale like any other.**
- **Settle a converter's behaviour on the binary, not on its output — again.** "MPXJ omits a ZERO `TotalSlack`" was a hypothesis about a writer read off its XML. One `javap` and a 40-line probe against `org.mpxj` on the intake `.mpp` answered it in a minute: the reader holds `0.0d` for all 62 (NULL for none), so absent ⇒ zero is the writer's rule and not a guess — and the same probe showed the rule is wider than the fix (completed tasks too), which became a registered row rather than a silent widening.
- **The mock's chips had to be DRIVEN before they could be named.** The DOM census called them "status-date chips"; clicking one re-pointed every KPI tile. A row of chips that SELECTS is a functionality claim, not a layout, and it changed the port from "the family's cursor" to "navigation to where that state is set". A census records what is there; only a driver records what it does.
- **Two of my own probes were weak before the tree was, and only the red run said so.** A chip-effect test that LOOPED over the chips it found passed on a page with no chips; `"Where it lands" not in page` failed on the pristine page because the chrome's story spine names that chapter. Neither would have been caught by the green run. Red-first is not ceremony; it is the only run in which an instrument's own defects are visible.
- **A fixture rewritten with `json.dumps` is a 870-line diff.** The first append to `case.json`'s note re-serialised the whole file; the fix was to re-detect the committed indent and `ensure_ascii` and write in the file's own format (one line changed). **Look at the diff of a golden before you commit it, every time.**
- **Stop before the unit that will not fit.** The token guardian read 65 % before the close; R-60 (converter + importer + engine + regenerated fixtures + corpus) was handed off with the facts found rather than started — including the one that changes its plan (the Revision-2 save Fuse analysed is in git history at `af4d154f`; ADR-0487 thought it lost). A half-built engine change in a PR is worse than none.
