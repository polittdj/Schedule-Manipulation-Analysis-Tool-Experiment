...
**Baselines to attribute against:** `-m parity` **198 passed / 0 failed in 6:53** in a separate worktree at the code commit `da4f7e67` (197 + the dated chain pin). The local full-suite run there was cut at 2,771 results (0 failed) by a container resume; CI's `test (3.11)` / `test (3.13)` on `4978fb4b` ran the full suite with the coverage gates and are green. Attribute the next local count against the previous unit's 5,634 green / 0 failed / 7 skipped **plus 16** (14 synthetic pins + the oracle's chain pin + the multicalendar test unchanged in count; the r58 test renamed) — expect **5,650 green / 0 failed / 7 skipped** and verify it, never transcribe it. Expected red at the code commit by construction: the handoff's version pin and the two latest-ADR pins (the code commit carries ADR-0509 and the bump, not the rotated docs). This unit adds 14 synthetic pins, one oracle test and one oracle row column. Version 1.0.275; highest ADR 0510; schema 2.16.0.
