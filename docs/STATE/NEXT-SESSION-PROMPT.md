...
**QUEUED BEHIND #677's MERGE — OR-18 (operator, 2026-09-15 ~14:25Z): the installed version at the top of the `/launch` Boot Screen.** Full entry and the unit's shape in `OPERATOR-REQUESTS.md` (OR-18). Do it as its own unit on the branch restarted from the squash: red-first on the pristine `/launch`, a server-rendered line under the CUI bar reading `chrome._ASSET_VERSION`, four themes in Chromium, two mutants by name, ADR-0494, v1.0.262, installers, the full gate, a draft PR. The global-header chip was OFFERED to the operator, not built — build it only if they say so.
