...
**QUEUED BEHIND #677's MERGE — OR-18 (operator, 2026-09-15 ~14:25Z): the installed version at the top of the `/launch` Boot Screen.** Full entry and the unit's shape in `OPERATOR-REQUESTS.md` (OR-18). Do it as its own unit on the branch restarted from the squash: red-first on the pristine `/launch`, a server-rendered line under the CUI bar reading `chrome._ASSET_VERSION`, four themes in Chromium, two mutants by name, ADR-0494, v1.0.262, installers, the full gate, a draft PR. The global-header chip is DIRECTED too (the operator agreed ~14:40Z): both surfaces in the one unit.

**OR-17 is SHIPPED IN PART (ADR-0493, v1.0.261) — do not re-open its shipped part.** Measured: the operator's build is v1.0.255 / 0.256 (the screenshot's placeholder and banner strings exist only there; `data-backend-only` absent) — nothing OR-16 shipped had been installed; the tool's key path holds on the REAL transport against a loopback fake gateway (`tests/web/test_gateway_wire.py`) and the DPAPI branch's Python holds against a fake `crypt32` (`tests/ai/test_dpapi_marshaling.py`); the real Win32 call runs in `installer-smoke.yml`'s Windows job (read the PR's `windows` check FIRST — that verdict is CI's, not the handoff's). Shipped: a one-shot Save receipt (replaced / kept, the length held, a misplaced-field warning), `tool version:` on /settings, the local-token label, a scheme-only challenge quoted. **What the operator owes on OR-17 (§4 of its ledger entry):** a v1.0.261 screenshot of /settings after pasting the CURRENT NASA AI Hub key into *Gateway API key* (the receipt + the banner's quoted reason settle the key vs the tool), V-4, and the one `Invoke-RestMethod …/v1/chat/completions` request that settles the catalog-401 hypothesis — **do NOT build a completion-shaped probe without it.**
