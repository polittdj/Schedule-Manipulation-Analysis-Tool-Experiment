...
⇢ NEXT — **branch FRESH from `origin/main` @ `47a5d758` or later; open a NEW draft PR.** RECOMMENDED FIRST (operator to confirm — the order is theirs): **CI-03's root-cause PR** — the caption sweep's `console@0.9` cell struck three times in 48 h (/forecast #626 · /curves #629 · /cei `main` #1717); test-instrument only (`tests/web/test_axis_titles_visual.py`: measure what the caption wait actually waits for at 0.9 scale under CI load — the per-page 5-s wait is `contextlib.suppress`ed, so a slow first paint yields ZERO captions and the cell reads "no captions rendered"; a `wait_for_function` on the caption selector with the page's own settle signal is the first candidate — never widen the timeout to silence it); red-first: reproduce the race under induced load (CPU throttling via CDP, or a parallel load) before fixing, and prove the fix by the same induced race going green. THEN **WP6** (ledger highs: CPM-01 `cpm.py:1316` · CPM-02 `driving_slack.py:314` · MC-02 · MC-03
