...
> ## STATUS (current) — **PR #626 (WP3 · M4, ADR-0454, v1.0.232) squash-merged to `main` @ `b631b41f` (2026-09-03 17:58Z, all seven CI checks green on its head `c33bf9fc`). Branch `claude/new-session-mc0a58` restarted from that `origin/main` (GitHub deleted the merged head); NO open PR at close. WP0/WP1/WP2/WP3 + the operator batch (ADR-0447..0453) ALL MERGED — the UI map has no queued rows. QC-1/QC-2 bind every session — ADR-0393, pinned by `tests/test_standing_rules.py`.**
> Highest ADR **0454**; version **1.0.232**; wheel + nine installers in lockstep on `main` — the operator re-downloads from `main` once (banner must read **v1.0.232**). Campaign queue: **WP4** (route-coverage instrument + the 08-26 `startup_failure` root-cause) is next — branch fresh from `origin/main`, open a new draft PR.
> **CI note for the record:** #626's `browser (measured-box proof)` job failed its first attempt on ONE cell of the 120-cell caption sweep (`console@0.9 /forecast: no captions rendered`) — a page the diff never touched, green on `main` the same morning with the identical Chrome 151 / chromium v1234 build, `/api/forecast` 10 ms locally. Established as not the PR's, commented on the PR, and it PASSED on the single permitted re-run (`rerun_failed_jobs` is refused while sibling jobs still run — wait for the run to complete). Not chased further; if it recurs on a `main` run it is a real race in `test_axis_titles_visual.py`'s 5-s per-page caption wait.
> **Also on `main` (operator, 2026-09-03 13:44 -0400, `04864b70` "Add files via upload"):** design-canvas intake refreshed under `00_REFERENCE_INTAKE/` (ASTROLABE / Command Deck / Mission Ops Redesign v2 / PortfolioMap `.dc.html`, a README and a 22-line intake HANDOFF) — reference material, not product code; read it before any design-related WP.
>
