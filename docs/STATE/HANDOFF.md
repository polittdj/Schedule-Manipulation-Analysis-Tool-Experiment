...
STATUS (current) — `main` @ **`9922e276`** (#662, the docs-only OR-13 registration, squash-merged by the operator 2026-09-10). **PR #663 is the OR-13 FIX, draft, and was 8/8 GREEN on head `7209e985`** (`check` · `browser` · `cui-guard` · `test 3.11` · `test 3.13` · `floor` · `linux` · `windows`) before `origin/main` moved under it. #662's squash then conflicted with this branch — which already carried those same two docs commits — and was **merge-resolved to this branch's side, proven rather than asserted**: `9922e276^{tree}` is byte-identical to `abf372cf^{tree}`, so main contributed nothing, and the merged tree equals the pre-merge tree `cf3f7659…`. One unit, **engine untouched**, red-first, mutation-tested **8/8 by name**. Highest ADR **0483**. Version **1.0.253** (wheel + nine installers rebuilt AFTER the last source edit; full suite started after THAT). QC-1/QC-2 bind every session — ADR-0393.
