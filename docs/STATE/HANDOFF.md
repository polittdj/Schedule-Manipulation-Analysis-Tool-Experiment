...
STATUS (current) — `main` @ **`26d821e3`** (#666, docs-only: the post-merge restart rule CORRECTED to a TREE comparison). Branch `claude/optimistic-ride-3qv2jc` restarted on that tip; the kickoff said `b195c7d3` and was one commit stale — **`git fetch origin` + `git log origin/main` before trusting any sha written here** (five consecutive kickoffs now). This session shipped **the design page owed five sessions running**: `/scorecards` (`setScreen('sk')`) wears the Control "Assessment Scorecards" artboard — **ADR-0484, v1.0.254**, engine untouched, red-first on a pristine worktree (11/12 layout · 4/5 browser), **19 mutations / 18 red by name** (the 19th behaviourally equivalent by design and recorded), four-theme render census pristine → final identical on every non-design key, `/card` and `/wbs` byte-identical across the shared-helper change. Full suite **5210 passed / 5 skipped** (the 3 installer failures were the shallow-graft guard refusing a pin my own `--depth 1` fetch made a boundary — unshallowed, 68/68), `-m parity` **96 passed**. Draft PR opened from this branch — its number and CI verdict are in the SESSION-LOG follow-up. Highest ADR **0484**. QC-1/QC-2 bind every session — ADR-0393.
