...
STATUS (current) — `main` @ **`4d53e070`** (#690, the docs-only merge record of #689, merged 19:39:33Z mid-gate), whose parent is **`ed977f29`** (#689, R-61 / ADR-0501). **`main`'s own runs for `ed977f29` are SETTLED — read to conclusion this session, not inherited: CI 1902 (`35133877304`) `completed` / SUCCESS, all six jobs (`cui-guard` 18:21:36Z · `browser` 18:38:57Z with the R-52 interop gate run-and-NOT-skipped on `main` itself · `floor` 18:46:41Z · `test (3.11)` 18:56:21Z · `test (3.13)` 19:01:31Z · `check` 19:01:36Z). NO installer-smoke run exists for `ed977f29` and that ABSENCE IS CORRECT, not a missing check** — #689 was docs+tests only and `installer-smoke.yml` is path-filtered to `installer/**`; the pattern is visible in the run list (`0b010936`, `93593ef5` and `4b213dd8` each have one, the four docs-only heads have none). **#690 (the docs-only merge record of #689) was MERGED at 19:39:33Z as `4d53e070`, while this unit's gate was running** — it was open when this session read it and is not now, so `origin/main` is **`4d53e070`** and this branch carries a merge of it (squash-merges make stacked branches conflict; CLAUDE.md's rule is merge-resolve). Its only content is two doc lines; the corrected STATUS line it wrote into the (c) section travelled with that section into `HANDOFF-ARCHIVE.md`. Highest ADR **0502**. Version **1.0.268** — this unit changed `src/`, so the wheel and the nine installers were rebuilt in lockstep. QC-1/QC-2 bind every session — ADR-0393.
