...
STATUS (current) — `main` @ **`2c549d8d`** (#691, R-57 / ADR-0502, **MERGED** 23:08Z; the squash is **TREE-IDENTICAL** to the reviewed head `96a987b1`, tree `4f5633e0…` on both, compared with `git rev-parse <sha>^{tree}` — steward §2's tree-hashes-first rule). #691 was **EIGHT OF EIGHT GREEN** on that head (CI run 35148410868 + installer-smoke 35148410690: `cui-guard` 20:45:18Z · `linux` 20:45:49Z · `windows` 20:50:17Z · `browser` 21:01:15Z · `floor` 21:08:31Z · `test (3.13)` 21:27:53Z · `test (3.11)` 21:31:13Z · `check` 21:31:18Z), zero review threads; eight is the correct set because the diff touches `installer/**`. **`main`'s OWN runs for `2c549d8d` are SETTLED — read to conclusion this session, not inherited: CI 1906 (`35160954016`) SUCCESS, all six jobs (`cui-guard` 23:08:44Z · `browser` 23:26:22Z with the R-52 interop gate run-and-NOT-skipped on `main` itself · `floor` 23:30:01Z · `test (3.13)` 23:39:21Z · `test (3.11)` 23:54:56Z · `check` 23:55:04Z), and installer-smoke 746 (`35160953983`) SUCCESS 23:13:38Z — which EXISTS here, correctly, because this merge rebuilt the installers (the contrast with #689's merge, where its absence was equally correct). Nothing about `2c549d8d` is outstanding.** The branch was restarted on the squash (`--prune` + `remote set-head` + `checkout -B`; post-merge safety check `HEAD^{tree}` vs `origin/main^{tree}` IDENTICAL) and the session unsubscribed. `chatgpt-codex-connector` posted its usage-limit notice on #691 — **review cover remains ABSENT**. Highest ADR **0502**. Version **1.0.268**. QC-1/QC-2 bind every session — ADR-0393.
