...
>
> 8. **Follow-up (ADR-0453, v1.0.231)** — on 1.0.230 Bottom = Months / "J, F, M" still rendered Quarters at their 7.4 px/month (the 8-px floor AND a 9-px label-blanking rule both fought the explicit one-glyph choice), and "Reset to default" looked dead because default and custom promote to the same rows. `fitPx` 7 for the letter label, blanking follows the label, `g-band-glyph` at 9 px, and an `effectiveNote` line under the dialog preview names each tier's effective unit and the promotion/push behind it. Two tests, RED-first.
>
> ## Operator-facing state
> Re-download once; the banner must read **v1.0.231**. Then: (a) which version was the blank-header screenshot from? (b) did the One-Pager `.pptx` open in PowerPoint? (c) on /analysis with two files: does the timeline header band across on one row per tier, and does scrolling feel smooth?
>
