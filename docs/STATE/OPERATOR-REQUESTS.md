...
## 2026-09-15 (c) — the 32-version Ask-the-AI answer put UID 152's "driving-path finish" 656 days past the Finish MS Project shows (USA IPMR Format 6_January 2024.mpp: 2028-02-22 vs 5/7/2026); the model's own memo blamed a network-finish substitution

### OR-20 — "I believe that the tool is experiencing some drift … verify and test and prove that the metrics that the tool is generating match those that SSI, Acumen Fuse, and Microsoft Project generate … find the root cause and test to see if you are correct, both pass and fail tests, and then create a fix that you test in a sandbox and then implement … verify that all other files are calculated correctly as well." · `SHIPPED (ADR-0497, v1.0.265)`

**Measured, on the operator's own IMS in the repo** (`ssi_uid152/Large_Test_File.mspdi.xml.gz`, "USA OTB Master IMS", 2,126 tasks, focus UID 152, with SSI's Directional Path export): the series builder read UID 152's OWN CPM early finish (2026-10-02 — its stored Finish, SSI's exported Finish, serial 46297.5), two years from the network finish (2028-09-28) — **the model's "populated with the network finish" diagnosis is refuted**; the tool's stored Start/Finish equal SSI's on 76 of 76 path members. **The root cause:** the drivers on each line are measured on the file's stored, progress-aware dates (ADR-0011 — SSI's axis), the finish beside them was the engine's logic-only CPM early finish, unlabelled, in the same shape as the SCHEDULE-LOGIC FINISH SERIES line; on this IMS the two disagree by more than a day on 50 of 1,723 activities (up to 106 d). For UID 152 they coincide in every save in the repo — the effect on the operator's 32 files is UNVERIFIED. **Shipped:** the line reports the stored Finish (MS Project's / SSI's), discloses the logic-only finish with its working-day gap wherever they disagree by ≥ 1 working day (compared on the project axis), says whose date it is, and the movement census runs on the stored Finish and counts the disagreeing versions. Red-first 8 of 10, two SSI oracles in `-m parity`, mutation 7 / 7 by name; **`-m parity` 118 / 0 on the pristine tree is the executable proof the committed corpus matches Fuse / SSI / MS Project** — its scope is the committed corpus, never the operator's production files, which can be checked only on the operator's machine.

**The operator owes (not blocking):** the DRIVING-PATH SERIES line for `USA IPMR Format 6_January 2024.mpp` from the session that produced the answer — the cited facts under the answer, or `/export/{fmt}/ask` of the last exchange (the transaction log keeps a prompt's hash only, Law 1) — OR simply the same question re-asked on v1.0.265: the line now states both dates wherever the engine's logic-only finish disagrees with the file, which settles whether the model transcribed the wrong series or the engine disagrees with that file by ~424 working days. Also which answer mode was set (the figures 697 / 1,286 / 589 were the model's own arithmetic across two axes; unrestricted is ungated by design, ADR-0361 — annotate flags such figures, strict discards them).

| # | Finding | Evidence | Status |
| --- | --- | --- | --- |
| OR-20b | **The SCHEDULE-LOGIC FINISH SERIES carries only the CPM network finish.** The model's 1,286-day figure subtracted the first version's CPM finish from the newest version's stored Finish of UID 6077 — two axes. | A per-version stored project finish (MS Project's latest Finish) beside the CPM one is the like-for-like series; `engine/version_series.py` + `ai/version_facts.py` + tests. | `OPEN (operator to decide)` |
| OR-20c | **The focus's stored Total Slack per version** is not on the line; the operator's question turns on whether UID 152 was near-critical in each file (513.5 d on Large_Test_File, 0 on Large_Test_File2). | One stored figure per line, read from `stored_total_float_minutes`, MS Project's own display division. | `OPEN (operator to decide)` |
| OR-20d | **The CPM-vs-stored-Finish residual census** on the IMS saves: 50 / 65 / 54 / 35 activities per save disagree by more than a day (worst-eight UIDs in the SESSION-LOG 2026-09-15 (e)). | The report's §3 R-rows name the mechanisms; no engine number moved in this unit. | `REGISTERED (measurement)` |
| OR-20e | **`working_minutes_between` credits a finish day whole on a calendar without day segments** (480 min between Fri 17:00 and Mon 08:00 on `Calendar()`). | Every MSPDI calendar carries segments; no shipped number known affected. | `REGISTERED (engine owners)` |
...
| V-5 | **PowerPoint opens the exported `.pptx`** (R-52 / OR-21, ADR-0498). LibreOffice Impress loads BOTH decks and parses every shape — measured 2026-09-15 on LibreOffice 24.2.7.2; the register's "does not load in LibreOffice 7" was an install with NO presentation import filter, which refuses a PowerPoint-authored deck identically. PowerPoint itself has never been in any container a session can reach, so it is the one reader still unverified. The package now carries the three parts a PowerPoint-authored deck carries and ours did not (`presProps` / `viewProps` / `tableStyles`), so its shape is no longer a variable | On v1.0.266: `/onepager` → **⤓ PPTX**, and `/onepager-compare` → **⤓ PPTX**; double-click each downloaded file. Say whether PowerPoint opens it, and if it complains, paste its exact wording (the repair dialog's text, never schedule content) | It opens: R-52 is closed in both readers and nothing further is owed. It refuses: the wording names the part, and the fix is a follow-on ADR against a named defect rather than a guess |
