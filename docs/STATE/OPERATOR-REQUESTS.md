...
| OR-11e | **Send `num_ctx` on the operator's say-so.** ADR-0480 measures truncation but never fixes it; only the operator can widen the window. | Must ship with the VRAM cost stated in the form — ollama/ollama#14073 records a 52 GB machine going unresponsive under a large default. | `OPEN` |
