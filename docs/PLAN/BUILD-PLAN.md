...
| **M15** | **`.pbix` reference + visual/metric enrichment** | ✔ **DONE (ADR-0030):** deck read locally (unzip → Report/Layout; DataModel is XPress9-compressed → measures reconstructed + documented); adopted float bands, completion performance, MEI, staleness, and the three-method finish forecast (`/forecast`); ambiguous measures (EPI/RatioMeasure/Start-and-Finish-Ratio) deferred pending a DAX export. |
