...
| A6 | `.pbix` reference: extract extra metrics/example visuals; expand/improve | `engine/metrics/{float_bands,completion_performance}.py`, `engine/forecast.py`, `/forecast` + report panels | `tests/engine/metrics/test_{float_bands,completion_performance}.py`, `tests/engine/test_forecast.py`, `tests/web/test_forecast_views.py` | ADR-0030 (deck read locally; DAX XPress9-compressed → reconstructed formulas documented) | M15 | ✔ **M15: float bands, completion performance, MEI, staleness, three-method finish forecast + drift; ambiguous measures deferred pending DAX export.** |
...
| B2 | Exact match to **Acumen v8.11.0** AND **SSI**; parity suite = gate | `engine/metrics/*`, `engine/driving_slack.py`, **`tests/parity/test_parity_gate.py`** | **parity gate (UID-keyed, `pytest -m parity`, CI step)** | `tests/fixtures/golden/{ssi_uid67,ssi_uid145,ssi_uid152,ssi_uid152_leveled,ssi_hardfile_uid155,project2_5}/case.json` | M6,M7,M8,M9 | ✔ **M9: consolidated parity acceptance gate live + CI-wired.** Exact: SSI UID-145 all-dependencies 108/108 + UID-67 Path-01 20/20 + UID-152 (2,126-task IMS) 76/76 (goldens refreshed per ADR-0112/0115, replacing the stale ssi_uid143); Acumen §A; §B 13/14; §C counts+BFC; §E Added/New-Critical/Finish-Slips/Completed/In-Progress + Net Finish Impact −148 (engine-CPM basis; reconciles day-exact to Fuse stored-basis −134, ADR-0108). Residuals **formally accepted + locked** (probe: neither pure-CPM nor stored-MSP reproduces them — ADR-0014): High Float +1, BSC %, SN04/06/07/09. Composite scores deferred (Acumen weighting unpublished; counts exact) |
...
| C2 | Target UID endpoint → trace driving path → Driving Slack in days == MSP+SSI | `engine/path_trace.py`,`driving_slack.py` | **SSI parity tests (Project5/UID 145 + UID 67; Large_Test_File/UID 152)** | `tests/fixtures/golden/{ssi_uid145,ssi_uid67,ssi_uid152}/case.json`; `tests/engine/test_driving_slack.py` | M6 | ✔ **M6 (goldens refreshed per ADR-0112/0115): anchored backward pass on progress-aware dates; Project5/UID 145 driving slack == SSI for all 108 UIDs exactly (all-dependencies, tier bands 2/3/8/95), UID-67 Path-01 20/20, UID-152 76/76 at 2,126-task scale.** |
