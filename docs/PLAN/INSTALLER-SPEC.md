...
**Status:** BUILT 2026-07-02 (`installer/install-tier{1,2,3}.ps1` + `installer/README-DISTRIBUTABLE.md`,
generated by `tools/installer/build_installers.py`; structure test-enforced in `tests/installer/`).
The operator authorized autonomous completion of all no-upload work while away, so §3 was answered
with the DOCUMENTED DEFAULTS below — each is a one-line edit in the generated files or a one-flag
regeneration if the operator wants different choices:

1. **OS: Windows 10/11 x64** (the spec's own assumption; the tool's MS-Project audience).
2. **GPU: warn-only detection** — the installer checks for a discrete GPU on Tier 2/3 and warns
   (never blocks); exact VRAM pinning awaits the operator's card model.
3. **Install-time internet: (A) online** — Python/Java/Ollama/model fetched from official sources
   at install time; the TOOL itself is embedded in the file (no internet needed for it). An
   offline bundle (B) remains available on request.
