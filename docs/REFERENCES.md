...
| `CEI` | (named in the build plan) **NOT implemented** — no single citable definition in this domain (cost-efficiency CPI=EV/AC would need an `actual_cost` field; or an Acumen-specific metric). Deferred rather than fabricated (LAW 2) | (none yet) | **source-pending** — definition + source must be pinned before implementation |
