...
the CPM network, and the Float-Ratio single-axis handling of an elapsed activity (audit NEW-1).
"""
...
def test_float_ratio_population_excludes_summary_and_inactive_and_scores_the_elapsed_task() -> None:
    ...
    # activity is scored without error on a single axis (NEW-1; the isolated 0.33 value is pinned
    # in test_float_ratio.py). Here we guard that summary/inactive don't leak into the population.
