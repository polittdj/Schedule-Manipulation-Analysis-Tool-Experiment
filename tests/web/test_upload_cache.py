...
def test_reupload_of_identical_bytes_is_a_cache_hit(
    ...
    client.post("/upload", files=[("files", ("v2.xml", payload, "text/xml"))])  # same bytes

    assert calls["n"] == 1  # the second upload was served from cache — parse ran only once
    assert set(st.schedules) == {"v1", "v2"}  # both stored under their own keys
    # the cache changes speed, never the answer: the restored version is byte-identical content
