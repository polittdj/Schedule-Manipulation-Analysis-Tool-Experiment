...
def test_reads_are_never_origin_gated(sc) -> None:  # type: ignore[no-untyped-def]
    _st, client = sc
    # a GET with a foreign Origin still serves (SOP/CSP govern reads; Origin-gating GETs
    # would break same-origin navigations, which omit the header entirely)
