...
def test_integrity_two_file_picker_compares_the_chosen_pair() -> None:
    ...
    # (ADR-0513): engine-derived counterfactual whose path crosses Hard_File's off-calendar tasks,
    # its started work AND (since ADR-0474) its crew-calendar bookings and stored leveling delays,
    # and whose started UID 187 (since ADR-0513) resumes only its REMAINING work after the restored
    # link instead of re-spanning its whole plan — see test_change_effects_integration.py for the
    # full adjudication (old value reproduced on the pre-change engine, new value read on this
    # tree in the page, the AI fact base and two probes).
    assert "+6 wd" in page
