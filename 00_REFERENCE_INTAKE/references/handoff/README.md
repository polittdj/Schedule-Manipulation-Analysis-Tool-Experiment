...
Story-driven nav (chapters 01–07 with per-chapter takeaways + "Continue"
segues), consistent chart language (takeaway headline · dotted reading grid ·
labeled DD line · same series colors everywhere), and per-view nav patterns
(left rail for dark views, top bar for daylight). Those are template-level
changes to app.py page shells — do them screen by screen using the prototype
as the spec; the calculations feeding each panel stay exactly as they are.
