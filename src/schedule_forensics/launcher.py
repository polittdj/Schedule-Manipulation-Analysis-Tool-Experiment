...
def main(
    ...
    *,
    ...
    Refuses a non-loopback ``host`` (CUI: local-only). ``serve``/``browser``/``timer`` are
    injectable for testing; by default they are ``uvicorn.run`` / ``webbrowser.open`` /
    ``threading.Timer``.
    """
    if not is_loopback_host(host):
        raise ValueError(f"refusing to bind non-loopback host {host!r} — the tool is local-only.")
    serve = serve or uvicorn.run
    ...
    print(f"Schedule Forensics — serving the dashboard at {url}  (Ctrl-C to stop)")
    if open_browser:
        timer(_BROWSER_DELAY, browser, args=(url,)).start()
    serve(create_app(), host=host, port=chosen_port, log_level="warning")
