...
def brief_blocks(brief: DiagnosticBrief, *, ai_is_local: bool) -> list[object]:
    """The brief as Word blocks (reports.docx) — same content as the page, verbatim.

    ``ai_is_local`` is REQUIRED evidence, not a default (DoD 001b): this exported exhibit
    prints a locality assurance, so every caller must supply the session's OBSERVED verdict
    (``not banner_for(config).cloud_active``). The brief's own content is engine-computed —
    the conditional sentence discloses the SESSION's AI posture, so an exhibit exported from
    a non-local session can never carry the unconditional local-generation claim.
    """
    ...
    blocks: list[object] = [
        ...
        Paragraph(
            ...
            if ai_is_local
            else "Generated by POLARIS² from the loaded schedule files; NOTE — this session's "
            "AI backend is configured for a non-local endpoint, so AI-generated prose in this "
            "session may have transited an external service. Every figure in this brief is "
            "engine-computed on this machine; every claim cites the schedule, UniqueID, and "
            "activity that substantiate it.",
            italic=True,
        ),
    ]
