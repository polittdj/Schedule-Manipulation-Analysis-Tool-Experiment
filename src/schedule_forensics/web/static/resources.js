/* Schedule Forensics — resource-loading histogram (ADR-0125).
 *
 * Reads the server-computed loading payload embedded in #resData and draws the selected resource's
 * monthly work-vs-capacity histogram: a bar per month (working days of booked work), a capacity tick
 * per month, and over-allocated months (load > capacity) drawn red. Dependency-free SVG, same-origin
 * only (air-gap) — nothing leaves the machine. The shared chartframe (#resChart is a .chart-host)
 * adds zoom + the hover call-out.
 */
"use strict";
...
(function () {
  ...
  function draw(res) {
    ...
    series.forEach(function (p, i) {
      ...
      // x labels: show every other month when crowded
    ...
    s.appendChild(txt(svg("text", { x: ml, y: H - 6, class: "res-xl" }),
      "bars = work booked (days); dash = monthly capacity; red = over-allocated"));
    host.appendChild(s);

    if (status) {
      status.textContent = res.total_days + " work-days total" +
        (res.over && res.over.length ? " · over-allocated in " + res.over.length + " month(s)" : "");
    }
  ...
  pick.addEventListener("change", function () { draw(selected()); });
  draw(selected());
})();
