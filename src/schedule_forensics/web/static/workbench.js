...
(function () {
  ...
  function renderRibbon() {
    ...
    // Native <caption> (decision B1, ADR-0326; shared helper ADR-0340): names the ribbon's
    // dimensions — the version columns carry only concrete per-version labels, so the column
    // DIMENSION is otherwise unnamed.
    SFGantt.tableCaption(table, "Selected metrics × schedule version");
    var thead = el("thead");
    var hr = el("tr");
  ...
  function renderGrid(panel) {
    ...
    // Native <caption> (B1, ADR-0326; shared helper ADR-0340): the drill's row unit.
    SFGantt.tableCaption(table, "Activities behind " + drill.name + " — one row per activity");
    var thead = el("thead");
    var hr = el("tr");
