...
1. **Data sovereignty (CUI).** No schedule content or derived metric ever leaves the machine; the AI is
   ...
   **ADR-0399 widened both detectors and added containers** (audit HOOK-01): the suffix chain also
   ...
   `tools/intake_manifest.py`, pinned by `tests/guards/test_intake_manifest.py`): 433 tracked files,
   99 of which carry an extension their bytes contradict — an inherited bulk-upload rotation that
   reached **neither the product nor the parity oracles**, and is now regression-guarded.
