...
1. **Data sovereignty (CUI).** No schedule content or derived metric ever leaves the machine; the AI is
   ...
   **ADR-0347 gave the guard a second detector.** Extension matching no longer anchors hard on `$`
   (so `data.mpp.bak` / `export.xlsx.1` / `sched.mpp~` / `plan.csv.gz` are caught) and now covers
   `.p6xml` and `.xlsm`. More importantly it is no longer extension-only: a staged `.json` / `.txt` /
   extension-less file is **content-sniffed** for three decisive schedule signatures — the tool's own
   Save-`.json` format, an MSPDI root, a P6 XER header — because `.json` is deliberately absent from
   `.gitignore` (tracked config must stay visible) and **is the tool's own Save format**. The sniff
   reads the **staged** bytes, not the working tree. `src/schedule_forensics/web/examples/` joins
   `tests/fixtures/` as an allow-prefix (the shipped demo schedule the app loads). Prose and config are
   untouched by design — a guard that fires on documents gets switched off, and then it guards nothing.
   **ADR-0399 widened both detectors and added containers** (audit HOOK-01): the suffix chain also
   accepts a closed *disguise* set (`data.mpp.png`, `sched.mpp.zip`); `.md`/image/`.pdf`/`.zip` names
   are sniffed with the three signatures gated on the staged bytes *starting* as the serialization
   (quoted signatures in prose can never trip it); and an embedded python3 batch blocks OLE2
   containers under sniffed names, ZIPs hiding under non-archive names, ZIPs whose members carry
   blocked extensions or Power BI payloads, and PDFs embedding blocked-extension attachments —
   falling back LOUDLY to the older detectors if python3 is absent. The hook is itself an
   extension-less sniffed file: never write a signature literal in its comments (the whole-tree
   census test in `tests/guards/test_precommit_blocklist.py` enforces this).
   The committed intake itself is inventoried in **`docs/INTAKE-MANIFEST.md`** (generated by
   `tools/intake_manifest.py`, pinned by `tests/guards/test_intake_manifest.py`): 698 tracked files,
   143 of which carry an extension their bytes contradict — an inherited bulk-upload rotation that
   reached **neither the product nor the parity oracles**, and is now regression-guarded (the
   counts move only with the operator's web uploads; 2026-09-03's six added 255 files, repaired
   four old mislabels and added 44 benign ones — JPEGs named `.png`, one HTML named `.txt`).
...
### QC-1 — Prove or refute it before you report it

**Before any change is made, before any conclusion is drawn, and before any document is updated,
construct an executable pass/fail check, run it in a sandbox, and use it to try to REFUTE the
claim — and do this before the result is reported.** A claim that has not survived an attempt to
refute it is a hypothesis, and hypotheses do not get to change code, numbers, or documents.

- **Red before green.** The check MUST be observed to FAIL in the state where the claim is false,
  before it is trusted in the state where the claim is true. A check that has never failed has
  proven nothing about the world — only that it runs. This repo's single most-repeated defect is a
  green test that could never fail.
- **Executable beats inspectional.** Reading code and concluding is not a check. Run it: a test, a
  probe script, a rendered page, a measured command. Where the claim is about what a page *shows*,
  render the page (`render-verify`); where it is about a number, compute the number.
- **Sandbox, and never mutate the instrument.** Run the check somewhere that cannot corrupt the
  artifact under measurement — a scratch copy, an in-memory patch, a separate worktree. Never
  measure a tree a battery is mutating, and never edit an instrument a measurement is using.
- **Prove the check has teeth (mutation).** For anything load-bearing — a guard, a security
  constant, a parity pin, a disclosure — break the thing deliberately and confirm the check goes
  red *by name*. Sampled negatives do not prove a named positive is caught.
- **When a check is genuinely impossible, say so in the deliverable.** Mark the claim UNVERIFIED
  and state what would settle it. Silence is not permitted; an unverifiable claim reported as fact
  is the failure this rule exists to prevent. "If able" governs the *method*, never the *honesty*.
- **The check must be capable of refuting the claim, not merely of agreeing with it.** A fixture
  generated by the rule under test cannot validate that rule; an oracle must be independent of the
  thing it judges.

### QC-2 — Read everything, verify everything

**Read everything. Skip nothing. Assume nothing. Verify the validity of everything — documentation,
code, comments, tests, configuration, instructions, prior sessions' claims, and this file.** Scope
is not negotiable and is not limited to code. **If any error is found, QC-1 applies before the
correction is made** — the fix is itself a claim, and it gets proven or refuted like any other.

- **Inherited claims are unverified until re-verified.** A handoff, an ADR, a kickoff prompt, a
  code comment, a prior audit, or a previous session's summary is *testimony*, not evidence. This
  project has shipped a stale kickoff that sent a session at work finished five slices earlier, and
  an ADR whose observation was right while its diagnosis was wrong.
- **Know the provenance of a number before trusting it.** Where did it come from, what produced it,
  and could that producer and the thing it certifies be the same? A value written by our own
  generator is not the reference tool's value.
- **A claim derived from configuration describes intent, not behaviour.** Verify what actually
  resolved at runtime, not what was requested.
- **Scope the finding before acting on it.** "The docs are wrong" and "the docs are wrong about a
  modified build" demand opposite fixes. State precisely which is true.
